...
1. Analytical Convolution: Instead of applying filters via direct matrix convolutions we use the fact that T[f*g]=T[f].T[g]( T[.]:= Fourier transform, * :=Convolution). So, we save processing time by multiplying the frequency spectrums instead of directly convolving.
